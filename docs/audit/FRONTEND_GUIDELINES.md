...
- 产物硬门禁：`dist` 全部 JS gzip 合计 ≤160 KiB（当前 149.3 KiB），CSS gzip 合计 ≤25 KiB（当前 18.0 KiB）；按 1 KiB = 1024 bytes 计算，超过 1 byte 即失败。
- 160 KiB 是批次 2 卡面系统的封顶值，不是目标值；新增功能仍须先做模块归因，并在每个批次重新执行构建与 `scan:dist`。
