...
当前版本：应用 `1.0.0`；模型 `3.0.0`；数据 `2026.08.14`。

## 产品能力

- 7 步移动优先流程：边界 → 基础范围 → 核心条件 → 维度库 → 敏感/娱乐 → 结果解释 → 分享。
- 69 个原子维度，明确区分硬筛选、相关硬条件、软偏好与娱乐项。
- 算法 API 的主人数只计算有可靠同口径参数的硬条件，并返回未量化硬条件、人数上限与不可用状态；现有结果页尚未消费这些 v3 字段，须由前端下一轮接入后才能向终端用户完整呈现。
- 收入、资产、住房、车辆、体型、学历、疾病等不会用不相容的宏观率硬扣人数；软偏好和娱乐项不减少人口池。
- 有一手常住人口锚点的城市才由算法 API 输出 C 级宽情景；其他城市返回 `unavailable`，不用历史约数冒充精确值。现有结果页尚未正确呈现这一状态。
- 关系情境算法 API 支持男找女、女找男、男找男、女找女，并把取向相容、当前单身、关系意愿与主人口分层；前端当前尚未接入。
