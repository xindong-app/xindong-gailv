...
当前版本：应用 `1.0.0`；模型 `4.0.0`；数据 `2026.08.14.3`。

## 产品能力

- 7 步移动优先流程：边界 → 基础范围 → 核心条件 → 维度库 → 敏感/娱乐 → 结果解释 → 分享。
- 69 个原子维度，明确区分硬筛选、相关硬条件、软偏好与娱乐项。
- `population` 可靠层只计算有可靠同口径参数的硬条件，并返回未量化硬条件、人数上限与不可用状态；学历按七普表 4-1 的18—50岁逐岁×性别直接人数进入该层。
- 底层新增 `comprehensivePopulation` 全条件综合情境：69个维度只要被主动选择且确实形成约束都会有计算路径；弱证据使用研究代理或明确的最大熵宽先验，并按17个相关簇避免机械连乘。学历是逐岁×性别互斥分层母集，院校和收入在学历单元内条件化，不再把学历当普通概率重复扣减。仅有通用开关、未记录具体方向/阈值时，API明确返回 `prior_sensitivity_only`，不能冒充可识别的精确人群集合；当前前端是否展示该字段由前端接入版本决定。
...
2. 新的硬筛选必须有口径、来源、公式、证据等级与边界；没有可靠人口比例时，可靠层保留为未量化硬条件并标人数上限。若进入全条件综合情境，必须明确登记为研究代理或分析者先验并给宽范围，不得伪装成调查发生率。
