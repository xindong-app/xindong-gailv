...
当前版本：应用 `1.0.0`；模型 `2.1.0`；数据 `2026.08.13`。

## 产品能力

- 7 步移动优先流程：边界 → 基础范围 → 核心条件 → 维度库 → 敏感/娱乐 → 结果解释 → 分享。
- 69 个原子维度，明确区分硬筛选、相关硬条件、软偏好与娱乐项。
- 硬条件显示保守/基准/乐观范围、证据等级、影响排行、放宽建议和模型分辨率。
- 收入/资产以及健康/生活方式在相关组内计算；软偏好和娱乐项不减少人口池。
- 分享图在本地 Canvas 生成。收入、资产、婚史、健康、关系边界与反向自评默认不分享；受反向自评影响的软契合/双向命中分数也不进入分享 DTO。
- 纯前端单页应用，无账号、无分析脚本、无第三方字体、无后端筛选 API。
...
乐观/基准/保守是启用模型组预设不确定度形成的敏感度范围，不是抽样校准的置信区间。模型可信度与单项证据等级分开展示，且不会高于最弱启用人口组的证据等级。低于模型分辨率表示“现有数据无法可靠区分”，不表示现实中绝对不存在。

## 贡献约束

1. 模型、数据或维度语义变化时同步更新版本、证据登记、测试和场景对比。
2. 新的硬筛选必须有口径、来源、公式、证据等级与边界；没有可靠人口比例时应归为软偏好或娱乐。
