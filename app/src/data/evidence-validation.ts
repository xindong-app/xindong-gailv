...
const evidenceEstimateSchema = object({
  baseline: number().finite().nullable(),
  optimistic: number().finite().nullable(),
  conservative: number().finite().nullable(),
  unit: enumSchema([
    'share',
    'people',
    'wan',
...
export const evidenceEntrySchema = object({
  id: string().min(1),
  dimensionId: string().min(1),
  dimensionName: string().min(1),
  definition: string().min(1),
  classification: enumSchema(['hard', 'correlated_hard', 'soft', 'entertainment']),
  applicablePopulation: string().min(1),
  geography: string().min(1),
  ageRange: object({ min: number().int().nullable(), max: number().int().nullable() }).strict(),
  sexBasis: string().min(1),
  dataYear: string().min(1),
  publisher: string().min(1),
  sourceTitle: string().min(1),
  sourceUrl: string().url(),
  retrievedAt: iso.date(),
  denominator: string().min(1),
  directValue: string().min(1),
  transformation: string().min(1),
  adjustments: array(string().min(1)),
  estimate: evidenceEstimateSchema,
  grade: enumSchema(['A', 'B', 'C', 'D']),
  limitations: array(string().min(1)).min(1),
  modelVersion: string().min(1),
  dataVersion: string().min(1),
  modelUse: enumSchema(['direct', 'anchor', 'calibration', 'excluded']),
}).strict()

export const evidenceRegistrySchema = object({
  dataVersion: string().min(1),
  modelVersion: string().min(1),
  retrievedAt: iso.date(),
  methodology: object({
    ...
    rules: array(string().min(1)).min(1),
  }).strict(),
  entries: array(evidenceEntrySchema).min(1),
...
export type EvidenceEntry = Infer<typeof evidenceEntrySchema>
export type EvidenceRegistry = Infer<typeof evidenceRegistrySchema>
...
export function parseEvidenceRegistry(input: unknown, buildDate: string): EvidenceRegistry {
  ...
  const buildDateResult = iso.date().safeParse(buildDate)
...
/** Exact full-registry lookup used by validation and runtime-alignment tests. */
