...
// 「X及以上」快捷组合: 一键映射对应档位并集
const EDUCATION_SHORTCUTS: Array<{ label: string; levels: readonly EducationId[] }> = [
  { label: '大专及以上', levels: ['junior_college', 'bachelor', 'master', 'doctorate'] },
  { label: '本科及以上', levels: ['bachelor', 'master', 'doctorate'] },
  { label: '硕士及以上', levels: ['master', 'doctorate'] },
]
    ...
    <section className="step-panel" aria-labelledby="core-title">
      ...
      <div className="criteria-grid">
        ...
        <article className="criteria-card criteria-wide has-sticker" data-kind="correlated">
          ...
          <div className="chip-row nested-row"><span>快捷组合</span>{EDUCATION_SHORTCUTS.map((shortcut) => { const on = shortcut.levels.every((id) => educationLevels.includes(id)); return <Chip key={shortcut.label} active={on} tone="pink" onClick={() => update((draft) => { draft.correlated.educationLevels = on ? draft.correlated.educationLevels.filter((id) => !shortcut.levels.includes(id)) : [...new Set([...draft.correlated.educationLevels, ...shortcut.levels])] })}>{shortcut.label}</Chip> })}</div>
          ...
          <FieldHelp>「大专/本科/硕士及以上」是一键并集快捷键；「本科（精确类别）」只算本科这一档。学历按七普表 4-1 逐岁×性别直接计入人数，多选按并集；院校层级以学历为条件做宽口径情景估算。清北 ⊂ C9 ⊂ 985 ⊂ 211。</FieldHelp>
        </article>
