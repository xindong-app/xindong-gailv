...
export function SensitiveFunStep({ selection, onChange, onNext }: { selection: ModelSelection; onChange: (next: ModelSelection) => void; onNext: () => void }) {
  ...
  return (
    <section className="step-panel" aria-labelledby="sensitive-title">
      <div className="step-heading"><span className="eyebrow">第四幕 · 彩蛋与边界</span><h2 id="sensitive-title" tabIndex={-1}>彩蛋随便玩，敏感的有护栏</h2><p>敏感人口条件与敏感偏好都默认折叠、默认不分享；星座和 MBTI 按最大熵先验凑热闹，证据等级最低，纯图一乐。</p></div>
      <section className="sensitive-box">
        ...
        {sensitiveOpen && <div id="sensitive-options">
          <div className="sensitive-hard-grid">
            <fieldset className="sensitive-hard-card"><legend><DimensionSticker className="legend-sticker" dimensionId="economy.house" />住房条件 · 计入综合估算</legend><Chip active={selection.correlated.housing.required} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.required = !draft.correlated.housing.required })}>要求有住房</Chip><div className="chip-row nested-row"><span>位置</span>{HOUSE_LOCATIONS.map(([id, label]) => <Chip key={id} active={selection.correlated.housing.location === id} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.location = draft.correlated.housing.location === id ? null : id; if (draft.correlated.housing.location) draft.correlated.housing.required = true })}>{label}</Chip>)}</div><div className="chip-row nested-row"><span>类型</span>{HOUSE_TYPES.map(([id, label]) => <Chip key={id} active={selection.correlated.housing.type === id} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.type = draft.correlated.housing.type === id ? null : id; if (draft.correlated.housing.type) draft.correlated.housing.required = true })}>{label}</Chip>)}</div><label className="compound-number" htmlFor="house-area"><span>最低面积（㎡）</span><input id="house-area" min="1" max="2000" placeholder="不限" type="number" value={selection.correlated.housing.minAreaSqm ?? ''} onChange={(event) => update((draft) => { draft.correlated.housing.minAreaSqm = event.target.value ? Number(event.target.value) : null; if (draft.correlated.housing.minAreaSqm) draft.correlated.housing.required = true })} /></label></fieldset>
            <fieldset className="sensitive-hard-card"><legend><DimensionSticker className="legend-sticker" dimensionId="economy.vehicle" />车辆条件 · 计入综合估算</legend><Chip active={selection.correlated.vehicle.required} tone="sun" onClick={() => update((draft) => { draft.correlated.vehicle.required = !draft.correlated.vehicle.required })}>要求有车</Chip><div className="chip-row nested-row"><span>价位并集</span>{CAR_BANDS.map(([id, label]) => <Chip key={id} active={selection.correlated.vehicle.priceBands.includes(id)} tone="sun" onClick={() => update((draft) => { draft.correlated.vehicle.priceBands = toggleArrayValue(draft.correlated.vehicle.priceBands, id); if (draft.correlated.vehicle.priceBands.length > 0) draft.correlated.vehicle.required = true })}>{label}</Chip>)}</div></fieldset>
            <fieldset className="sensitive-hard-card"><legend><DimensionSticker className="legend-sticker" dimensionId="health.chronic" />健康与外形 · 宽口径计入</legend><div className="chip-row"><Chip active={selection.correlated.healthCriteria.includes('no_major_chronic')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_major_chronic') })}>无慢性病</Chip><Chip active={selection.correlated.healthCriteria.includes('no_myopia')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_myopia') })}>不近视</Chip><Chip active={selection.correlated.hairCriteria.includes('full_hair')} tone="sun" onClick={() => update((draft) => { draft.correlated.hairCriteria = toggleArrayValue(draft.correlated.hairCriteria, 'full_hair') })}>不脱发</Chip></div><p>这些条件都按各自的宽口径证据进入综合估算，证据等级随结果公示：慢性病和近视缺全国统一口径，脱发只靠六城社区研究——范围会给得很宽，看个量级就好，别当精确值。</p></fieldset>
            <fieldset className="sensitive-hard-card"><legend><DimensionSticker className="legend-sticker" dimensionId="appearance.body_type" />体型与烟酒 · 全部计入</legend><div className="chip-row">{BODY_OPTIONS.map(([id, label]) => <Chip key={id} active={selection.correlated.bodyTypes.includes(id)} tone="sun" onClick={() => update((draft) => { draft.correlated.bodyTypes = toggleArrayValue(draft.correlated.bodyTypes, id) })}>{label}</Chip>)}</div><div className="chip-row nested-row"><span>生活习惯</span><Chip active={selection.correlated.smoking === 'non_smoker'} tone="sun" onClick={() => update((draft) => { draft.correlated.smoking = draft.correlated.smoking === 'non_smoker' ? 'any' : 'non_smoker' })}>当前不吸烟</Chip><Chip active={selection.correlated.drinking === 'not_regular'} tone="sun" onClick={() => update((draft) => { draft.correlated.drinking = draft.correlated.drinking === 'not_regular' ? 'any' : 'not_regular' })}>过去 30 天未饮酒</Chip><Chip active={selection.correlated.drinking === 'none'} tone="sun" onClick={() => update((draft) => { draft.correlated.drinking = draft.correlated.drinking === 'none' ? 'any' : 'none' })}>过去 12 个月未饮酒</Chip></div><p>体型按 BMI 分类并集做宽口径估算，主观标签范围偏宽；烟酒按全国调查回溯期口径真实计入，联合用 Fréchet 界。回溯期口径不代表终身状态。</p></fieldset>
          </div>
      ...
      <section className="fun-box" aria-labelledby="fun-title"><div className="fun-heading"><span className="class-badge">纯娱乐</span><h3 id="fun-title">玄学在这里单独玩</h3><p>玄学也会上场砍人——按最大熵先验演算，证据等级 D，看着玩别当真。</p></div>
