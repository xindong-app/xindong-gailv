      ...
      <section className="sensitive-box">
        ...
        {sensitiveOpen && <div id="sensitive-options">
          <div className="sensitive-hard-grid">
            <fieldset className="sensitive-hard-card"><legend>住房条件 · 相关硬条件</legend><Chip active={selection.correlated.housing.required} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.required = !draft.correlated.housing.required })}>要求有住房</Chip><div className="chip-row nested-row"><span>位置</span>{HOUSE_LOCATIONS.map(([id, label]) => <Chip key={id} active={selection.correlated.housing.location === id} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.location = draft.correlated.housing.location === id ? null : id; if (draft.correlated.housing.location) draft.correlated.housing.required = true })}>{label}</Chip>)}</div><div className="chip-row nested-row"><span>类型</span>{HOUSE_TYPES.map(([id, label]) => <Chip key={id} active={selection.correlated.housing.type === id} tone="sun" onClick={() => update((draft) => { draft.correlated.housing.type = draft.correlated.housing.type === id ? null : id; if (draft.correlated.housing.type) draft.correlated.housing.required = true })}>{label}</Chip>)}</div><label className="compound-number" htmlFor="house-area"><span>最低面积（㎡）</span><input id="house-area" min="1" max="2000" placeholder="不限" type="number" value={selection.correlated.housing.minAreaSqm ?? ''} onChange={(event) => update((draft) => { draft.correlated.housing.minAreaSqm = event.target.value ? Number(event.target.value) : null; if (draft.correlated.housing.minAreaSqm) draft.correlated.housing.required = true })} /></label></fieldset>
            <fieldset className="sensitive-hard-card"><legend>车辆条件 · 相关硬条件</legend><Chip active={selection.correlated.vehicle.required} tone="sun" onClick={() => update((draft) => { draft.correlated.vehicle.required = !draft.correlated.vehicle.required })}>要求有车</Chip><div className="chip-row nested-row"><span>价位并集</span>{CAR_BANDS.map(([id, label]) => <Chip key={id} active={selection.correlated.vehicle.priceBands.includes(id)} tone="sun" onClick={() => update((draft) => { draft.correlated.vehicle.priceBands = toggleArrayValue(draft.correlated.vehicle.priceBands, id); if (draft.correlated.vehicle.priceBands.length > 0) draft.correlated.vehicle.required = true })}>{label}</Chip>)}</div></fieldset>
            <fieldset className="sensitive-hard-card"><legend>健康与外形 · 相关硬条件 / 软偏好</legend><div className="chip-row"><Chip active={selection.correlated.healthCriteria.includes('no_major_chronic')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_major_chronic') })}>慢性病信息偏好（软）</Chip><Chip active={selection.correlated.healthCriteria.includes('no_myopia')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_myopia') })}>不近视（软偏好）</Chip><Chip active={selection.correlated.hairCriteria.includes('full_hair')} tone="sun" onClick={() => update((draft) => { draft.correlated.hairCriteria = toggleArrayValue(draft.correlated.hairCriteria, 'full_hair') })}>无雄激素性脱发</Chip></div><p>“无重大慢性病”缺少定义一致的联合比例，“不近视”也缺成人同口径数据，两者只进入契合示意；脱发条件按年龄、性别研究锚点进入相关人口组。</p></fieldset>
            <fieldset className="sensitive-hard-card"><legend>体型与烟酒 · 相关硬条件</legend><div className="chip-row">{BODY_OPTIONS.map(([id, label]) => <Chip key={id} active={selection.correlated.bodyTypes.includes(id)} tone="sun" onClick={() => update((draft) => { draft.correlated.bodyTypes = toggleArrayValue(draft.correlated.bodyTypes, id) })}>{label}</Chip>)}</div><div className="chip-row nested-row"><span>生活习惯</span><Chip active={selection.correlated.smoking === 'non_smoker'} tone="sun" onClick={() => update((draft) => { draft.correlated.smoking = draft.correlated.smoking === 'non_smoker' ? 'any' : 'non_smoker' })}>当前不吸烟</Chip><Chip active={selection.correlated.drinking === 'not_regular'} tone="sun" onClick={() => update((draft) => { draft.correlated.drinking = draft.correlated.drinking === 'not_regular' ? 'any' : 'not_regular' })}>过去 30 天未饮酒</Chip><Chip active={selection.correlated.drinking === 'none'} tone="sun" onClick={() => update((draft) => { draft.correlated.drinking = draft.correlated.drinking === 'none' ? 'any' : 'none' })}>过去 12 个月未饮酒</Chip></div><p>体型多选按并集；烟酒与健康在同一相关组校正，不做独立重复扣减。这些是调查回溯期口径，不代表终身状态。</p></fieldset>
          </div>
