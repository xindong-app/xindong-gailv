      ...
      <section className="sensitive-box">
        ...
        {sensitiveOpen && <div id="sensitive-options">
          <div className="sensitive-hard-grid">
            ...
            <fieldset className="sensitive-hard-card"><legend><DimensionSticker className="legend-sticker" dimensionId="health.chronic" />健康与外形 · 硬边界不砍人</legend><div className="chip-row"><Chip active={selection.correlated.healthCriteria.includes('no_major_chronic')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_major_chronic') })}>慢性病信息偏好（软）</Chip><Chip active={selection.correlated.healthCriteria.includes('no_myopia')} tone="peach" onClick={() => update((draft) => { draft.correlated.healthCriteria = toggleArrayValue(draft.correlated.healthCriteria, 'no_myopia') })}>不近视（软偏好）</Chip><Chip active={selection.correlated.hairCriteria.includes('full_hair')} tone="sun" onClick={() => update((draft) => { draft.correlated.hairCriteria = toggleArrayValue(draft.correlated.hairCriteria, 'full_hair') })}>无雄激素性脱发</Chip></div><p>“无重大慢性病”缺少定义一致的联合比例，“不近视”也缺成人同口径数据，两者只进入契合示意；脱发条件保留为硬边界但不砍人——六城社区研究的口径不足以代表全国。</p></fieldset>
