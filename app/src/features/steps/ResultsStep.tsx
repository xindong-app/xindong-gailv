  ...
  return (
    <section className="step-panel results-step" aria-labelledby="results-title">
      ...
      <div className="result-detail-grid">
        <article className="detail-card"><div className="detail-head"><div><span className="eyebrow">边际重算 · 综合口径</span><h3>哪些条件最“狠”</h3></div><span className="count-badge">{result.comprehensivePopulation.impacts.length}</span></div>
          {result.comprehensivePopulation.impacts.length === 0 ? <p className="empty-inline">当前只有基础范围，没有额外限制项。</p> : <ol className="impact-list">{result.comprehensivePopulation.impacts.slice(0, 6).map((impact) => <li key={impact.dimensionId}><div><b>{impact.label}</b><span><EvidenceStatusBadge grade={impact.evidenceGrade} /> · 保留 {(impact.retention * 100).toFixed(1)}%</span></div><strong>− {formatCount(impact.marginalLoss)}</strong></li>)}</ol>}
        </article>
      ...
      <article className="model-boundaries"><h3>模型知道什么，也知道自己不知道什么</h3><ul>{result.explanation.map((item) => <li key={item}>{item}</li>)}</ul><p>可信度 {result.confidence.grade}（{Math.round(result.confidence.score * 100)}% 方法评分）· 分辨率下限 {formatCount(result.comprehensivePopulation.resolutionFloor)}</p></article>
    </section>
  )
}
