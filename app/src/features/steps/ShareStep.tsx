...
import { buildChallengeText, copyChallenge } from '../../share'
...
export function ShareStep({ result, selection, onShare, onClearSession }: { result: ModelResult; selection: ModelSelection; onShare: () => void; onClearSession: () => void }) {
  ...
  const handleCopyChallenge = async () => {
    setCopied(await copyChallenge())
  }

  return (
    <section className="step-panel" aria-labelledby="share-title">
      <div className="step-heading"><span className="eyebrow">最终关 · 生成战报</span><h2 id="share-title" tabIndex={-1}>战报出炉前，先过一道安检</h2><p>生成前会列出公开字段。收入、资产、婚史、健康、关系边界和反向自评默认不进入分享；人数、地区、年龄、条件与娱乐指数都可隐藏。</p></div>
      <div className="share-landing-grid"><article className="share-poster-preview"><div className="share-poster-head"><span>择偶条件分析战报</span><small>条件组合分析</small></div><div className="poster-number">{result.comprehensivePopulation.displayShort}</div><p>{selection.target.age.min}–{selection.target.age.max} 岁 · {selection.target.gender === 'male' ? '男性' : '女性'}</p><div className="poster-score"><span>娱乐 {result.scores.entertainment}/100</span></div><footer>模型 {result.versions.modelVersion} · 数据 {result.versions.dataVersion}<br />仅供娱乐参考 · 非官方结论</footer></article>
      ...
      <article className="challenge-card" aria-labelledby="challenge-title">
        <div className="challenge-card-copy">
          <span className="eyebrow">裂变玩法 · 不带你任何数据</span>
          <h3 id="challenge-title">🆚 发给朋友，测测 TA 的缘分难度</h3>
          <blockquote>{challenge}</blockquote>
        </div>
        <button className="button button-secondary" type="button" onClick={handleCopyChallenge}>
          {copied ? '已复制，去群里 @TA 吧' : '复制挑战书'}
        </button>
      </article>
